...
type HostBindingReconciler struct {
	Client client.Client
	Cache  cache.Cache
}

func (reconciler *HostBindingReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	_ = json.Unmarshal([]byte(vmi.Annotations[annotation.VinkVirtualmachineinstanceHost.Name]), &oldIPs)
	...
	vmi.Annotations[annotation.VinkVirtualmachineinstanceHost.Name] = string(value)
...
func (reconciler *HostBindingReconciler) SetupWithManager(mgr ctrl.Manager) error {
