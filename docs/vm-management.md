...
支持为虚拟机配置池大小，当池大于 0 时，系统将自动创建 n 个相同的虚拟机，实现高可用部署，确保业务连续性和可靠性。

![虚拟机高可用](./images/虚拟机高可用.png)

创建完成高可用的虚拟机之后，您可以前往 `虚拟资源` > `虚拟机池`，进入 `虚拟机池列表` 页面。

![虚拟机高可用-虚拟机池列表](./images/虚拟机高可用-虚拟机池列表.png)

前往 `虚拟资源` > `虚拟机`，进入 `虚拟机列表` 页面，查看虚拟机池中的一组虚拟机。

![虚拟机高可用-虚拟机列表](./images/虚拟机高可用-虚拟机列表.png)
